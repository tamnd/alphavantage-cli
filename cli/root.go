// Package cli assembles the alphavantage command tree from the alphavantage
...
// NewApp assembles the kit application from the alphavantage domain. The
...
//
// To add a command, declare it in alphavantage/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := alphavantage.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(alphavantage.Domain{}).Register(app)
